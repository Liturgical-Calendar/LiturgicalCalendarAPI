...
# server PHP process so the bootstrap hook in bin/LitCalTestServer.php can
# collect per-message coverage. No-op in normal development / production.
if [ -n "${PCOV_SERVER_COVERAGE_DIR:-}" ]; then
  php -d pcov.enabled=1 -d pcov.directory=src bin/LitCalTestServer.php > /dev/null 2>&1 &
else
  php bin/LitCalTestServer.php > /dev/null 2>&1 &
fi
