...
The idea is to turn this into a self-contained class that will allow for a number of operations, such as retrieving the liturgical information for a specific date or for a time period other than for the full calendar year...

Included is an SQL table dump **liturgy.sql** which contains fixed date festivities, memorials and optional memorials of the General Roman Calendar. The PHP script aids itself with these tables to generate the liturgical calendar. The table itself does not contain all fixed date solemnities since some of these are defined in the PHP script itself, nor does it contain movable feasts which necessarily are calculated in the script.
