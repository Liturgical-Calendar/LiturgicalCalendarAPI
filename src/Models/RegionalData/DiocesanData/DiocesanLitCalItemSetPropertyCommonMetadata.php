...
final class DiocesanLitCalItemSetPropertyCommonMetadata extends LiturgicalEventMetadata
{
    ...
    {
        if (false === property_exists($data, 'since_year') || false === property_exists($data, 'property') || $data->property !== 'common') {
            throw new \ValueError('`since_year` and `property` are required for an `action` of `setProperty`, and `property` must have a value of `common`');
        }
        return new static(
            $data->since_year,
    ...
    {
        if (false === isset($data['since_year']) || false === isset($data['property']) || $data['property'] !== 'common') {
            throw new \ValueError('`since_year` and `property` are required for an `action` of `setProperty`, and `property` must have a value of `common`');
        }
        return new static(
            $data['since_year'],
