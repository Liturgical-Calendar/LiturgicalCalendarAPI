...
final class DiocesanLitCalItemSetPropertyNameMetadata extends LiturgicalEventMetadata
{
    ...
    protected static function fromObjectInternal(\stdClass $data): static
    {
        if (false === property_exists($data, 'since_year') || false === property_exists($data, 'property') || $data->property !== 'name') {
            throw new \ValueError('`since_year` and `property` are required for an `action` of `setProperty`, and `property` must have a value of `name`');
        }
        return new static(
            $data->since_year,
