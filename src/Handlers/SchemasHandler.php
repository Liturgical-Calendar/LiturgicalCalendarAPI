    ...
    public function produceResponse(array $requestPathParts = []): never
    {
        ...
        switch ($pathParamCount) {
            case 0:
                ...
                foreach ($it as $f) {
                    $schemaIndex->litcal_schemas[] = Router::$apiPath . Route::SCHEMAS->value . '/' . $f->getFilename();
                }
                ...
                break;
            case 1:
                if (file_exists(JsonData::SCHEMAS_FOLDER->path() . '/' . $requestPathParts[0])) {
                    $response = file_get_contents(JsonData::SCHEMAS_FOLDER->path() . '/' . $requestPathParts[0]);
                    if ($response === false) {
