...
final class PermissionAdminHandler extends AbstractHandler
{
    ...
    private function revokePermission(
        ...
        // The named tuple stays FIRST: the cascade decision below reads $outboxIds[0], and it is
        // the object the admin asked about that the response reports on.
        $outboxRows = [
     ...
     * `fga_object`, so it gets a distinct key for free — and each half of the pair therefore
     * retries, succeeds and dedupes independently of the other.
     *
     * `cascade_object_type` is the row's OWN object type, not the one the admin named. The
     * consumer-side reconciler reads it to decide which roles a drained delete could have emptied,
     * and the two halves of a pair are on different types by construction.
     *
     * @param  list<string> $cascadeRoleCandidates
     * @return array{
     ...
     * }
     */
    private static function revokeOutboxRow(
        string $adminUserId,
        string $fgaUser,
        string $bareUserId,
        string $relation,
        ...
        return [
            ...
            'fga_relation'    => $relation,
            'fga_object'      => $fgaObject,
            'idempotency_key' => "permission_revoke:{$adminUserId}:delete_tuple:{$fgaUser}:{$relation}:{$fgaObject}",
            'metadata'        => [
                'admin_user'              => "user:{$adminUserId}",
