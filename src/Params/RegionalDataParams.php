...
class RegionalDataParams implements ParamsInterface
{
    ...
    public function __construct()
    {
        ...
        $metadata = Utilities::jsonUrlToObject(Router::$apiPath . Route::CALENDARS->value);
    ...
    private function checkNationalCalendarConditions(array $params): string
    {
        ...
        if (count(array_intersect($requiredKeys, $givenKeys)) !== count($requiredKeys)) {
            RegionalDataHandler::produceErrorResponse(
                StatusCode::BAD_REQUEST,
                'Missing required parameter(s) `' . implode('`, `', array_diff($requiredKeys, $givenKeys)) . '`'
            );
        }

        if (RegionalDataHandler::$Core->getRequestMethod() === RequestMethod::PUT) {
        ...
        } elseif (RegionalDataHandler::$Core->getRequestMethod() === RequestMethod::DELETE) {
        ...
        if (false === in_array(RegionalDataHandler::$Core->getRequestMethod(), [RequestMethod::DELETE, RequestMethod::PUT], true)) {
            ...
            if (array_key_exists('locale', $params)) {
                ...
                if (
                    ...
                    $message = "Invalid value {$params['locale']} for param `locale`, valid values for nation {$params['key']} are: "
                                . implode(', ', $validLangs);
                    RegionalDataHandler::produceErrorResponse(StatusCode::BAD_REQUEST, $message);
                }
                ...
                RegionalDataHandler::produceErrorResponse(StatusCode::BAD_REQUEST, '`locale` param or `Accept-Language` header required for Wider Region calendar data');
            }
        }
    ...
    private function checkDiocesanCalendarConditions(array $params): string
    {
        ...
        if (RegionalDataHandler::$Core->getRequestMethod() !== RequestMethod::PUT) {
            ...
            if (RegionalDataHandler::$Core->getRequestMethod() !== RequestMethod::DELETE) {
                ...
                if (array_key_exists('locale', $params)) {
                    ...
                    RegionalDataHandler::produceErrorResponse(StatusCode::BAD_REQUEST, '`locale` param or `Accept-Language` header required for Diocesan calendar data when request method is GET, POST, or PATCH');
                }
            }
        } else {
    ...
    private function checkWiderRegionCalendarConditions(array $params)
    {
        ...
        if (
            ...
            && RegionalDataHandler::$Core->getRequestMethod() !== RequestMethod::PUT
        ) {
            ...
            RegionalDataHandler::produceErrorResponse(StatusCode::NOT_FOUND, $message);
        }
        ...
        if (array_key_exists('locale', $params)) {
            $params['locale'] = \Locale::canonicalize($params['locale']);
            if (
                RegionalDataHandler::$Core->getRequestMethod() === RequestMethod::PUT // short circuit for PUT requests that don't need to validate against existing locales
                || null !== $this->i18nRequest // short circuit for i18n requests
                || in_array($params['locale'], $validLangs, true)
            ) {
                $this->locale = $params['locale'];
    ...
    public function setParams(array $params): void
    {
        ...
        if (in_array(RegionalDataHandler::$Core->getRequestMethod(), [RequestMethod::GET, RequestMethod::POST], true)) {
        ...
        if (in_array(RegionalDataHandler::$Core->getRequestMethod(), [RequestMethod::PUT, RequestMethod::PATCH], true)) {
            if (
                ...
                RegionalDataHandler::produceErrorResponse(StatusCode::BAD_REQUEST, 'Cannot create or update Calendar data without a payload');
            } else {
