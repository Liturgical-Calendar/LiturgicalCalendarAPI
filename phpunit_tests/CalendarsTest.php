...
final class CalendarsTest extends ApiTestCase
{
    ...
    private function assertionsForCalendarObject(object $data): void
    {
        ...
        $this->assertObjectHasProperty('national_calendars', $data->litcal_metadata, 'JSON should have a "national_calendars" property under "litcal_metadata"');
        $this->assertObjectHasProperty('national_calendars_keys', $data->litcal_metadata, 'JSON should have a "national_calendars_keys" property under "litcal_metadata"');
        $this->assertObjectHasProperty('diocesan_calendars', $data->litcal_metadata, 'JSON should have a "diocesan_calendars" property under "litcal_metadata"');
        $this->assertObjectHasProperty('diocesan_calendars_keys', $data->litcal_metadata, 'JSON should have a "diocesan_calendars_keys" property under "litcal_metadata"');
        $this->assertObjectHasProperty('diocesan_groups', $data->litcal_metadata, 'JSON should have a "diocesan_groups" property under "litcal_metadata"');
        $this->assertObjectHasProperty('wider_regions', $data->litcal_metadata, 'JSON should have a "wider_regions" property under "litcal_metadata"');
        $this->assertObjectHasProperty('wider_regions_keys', $data->litcal_metadata, 'JSON should have a "wider_regions_keys" property under "litcal_metadata"');
        $this->assertObjectHasProperty('locales', $data->litcal_metadata, 'JSON should have a "locales" property under "litcal_metadata"');
        $this->assertIsArray($data->litcal_metadata->national_calendars, 'national_calendars should be an array');
        $this->assertIsArray($data->litcal_metadata->national_calendars_keys, 'national_calendars_keys should be an array');
        $this->assertIsArray($data->litcal_metadata->diocesan_calendars, 'diocesan_calendars should be an array');
        $this->assertIsArray($data->litcal_metadata->diocesan_calendars_keys, 'diocesan_calendars_keys should be an array');
        $this->assertIsArray($data->litcal_metadata->diocesan_groups, 'diocesan_groups should be an array');
        $this->assertIsArray($data->litcal_metadata->wider_regions, 'wider_regions should be an array');
        $this->assertIsArray($data->litcal_metadata->wider_regions_keys, 'wider_regions_keys should be an array');
        $this->assertIsArray($data->litcal_metadata->locales, 'locales should be an array');

        // Run assertions on each national_calendars item
        foreach ($data->litcal_metadata->national_calendars as $national_calendar) {
        ...
        foreach ($data->litcal_metadata->diocesan_calendars as $diocesan_calendar) {
        ...
        foreach ($data->litcal_metadata->national_calendars_keys as $key) {
        ...
        foreach ($data->litcal_metadata->diocesan_calendars_keys as $key) {
        ...
        foreach ($data->litcal_metadata->diocesan_groups as $group) {
            $this->assertIsObject($group, 'Each element of diocesan_groups should be an object');
            $this->assertObjectHasProperty('group_name', $group, 'Each item in diocesan_groups should have a "group_name" property');
            $this->assertIsString($group->group_name, 'group_name should be a string');
            $this->assertObjectHasProperty('dioceses', $group, 'Each item in diocesan_groups should have a "dioceses" property');
            $this->assertIsArray($group->dioceses, 'dioceses should be an array');
            foreach ($group->dioceses as $diocese) {
                $this->assertIsString($diocese, 'Each element of dioceses should be a string');
                $this->assertContains(
                    $diocese,
                    self::$dioceseIDs,
        ...
        foreach ($data->litcal_metadata->wider_regions as $wider_region) {
        ...
        foreach ($data->litcal_metadata->wider_regions_keys as $key) {
        ...
        foreach ($data->litcal_metadata->locales as $locale) {
            $this->assertIsString($locale, 'Each element of locales should be a string');
            $this->assertMatchesRegularExpression(self::LOCALE_PATTERN, $locale, 'Each element of locales should be a valid locale code');
        }
    }
